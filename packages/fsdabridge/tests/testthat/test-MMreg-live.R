test_that("MMreg wrapper agrees with MATLAB/FSDA", {
  skip_if_not(
  ...
  h = start_engine("MMreg", fsda_root = fsda_root)
  on.exit(stop_engine(h), add = TRUE)

  ref_dir = system.file("extdata", "MMreg", package = "fsdabridge")
  wool_path = file.path(ref_dir, "wool.csv")
  beta_path = file.path(ref_dir, "MMreg_beta.csv")
  ...
  expect_equal(as.character(out$class), "MMreg")
